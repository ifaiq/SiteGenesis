...
function execute() {
  ...
  var message;
  httpClient.open("GET", "https://v2.jokeapi.dev/joke/Any");
  ...
  if (httpClient.statusCode == 200) {
    ...
    Logger.info("Delivery message print" + message);
  }
}

function sentJokeApi(productId) {
  var product = ProductMgr.getProduct(productId);
  var apiResult = {};
  var result = {};
  apiResult = execute();
  Logger.info("Delivery message print" + apiResult);

  if (product.custom.callApi) {
    result.error = true;
