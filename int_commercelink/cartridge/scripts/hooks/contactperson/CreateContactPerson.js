 ...
 *
 *
 * @param {object} contactperson The ContactPerson to be created
 */
exports.CreateContactPerson = function (args) {
	var contactPersonObj = args.contactPersonObj;
	var returnStatus;
	if(CommerceLinkFactory.Preferences['enableRealTimeContactPersonCreation']) {
		try {
			/***********************************************
			*		Build Request & Call Service		   *
			***********************************************/
			// Creating Request Pay load
			var requestDataContainer  = ContactPersonFactory.buildCreateContactPersonRequestObject({
			...
			//success case
    			...
    			Transaction.wrap(function () {
    				if(responseObj.status == "true") {
    					...
    					//setting hook response
        				returnStatus = new Status(Status.ERROR, CommerceLinkFactory.STATUS_CODES.CONTACT_PERSON.CREATE.RESPONSE_ERROR);
        				returnStatus.addDetail(CommerceLinkFactory.STATUS_CODES.RESPONSE_OBJECT, result.object);
    				}
    			});
			}
