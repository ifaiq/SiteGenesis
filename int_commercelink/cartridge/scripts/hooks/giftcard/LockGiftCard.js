 ...
 *
 *
 * @param {object} giftCard Lock the existing GiftCard
 */
exports.LockGiftCard = function (giftCard) {
	var returnStatus;
	if(CommerceLinkFactory.Preferences['enableRealTimeLockGiftCard']) {
		try {
			/***********************************************
			...
			***********************************************/
			...
			//success case
				...
				if(responseObj.Status == "true") {
    				...
    				//setting hook response
    				returnStatus = new Status(Status.ERROR, CommerceLinkFactory.STATUS_CODES.GIFT_CARD.LOCK.RESPONSE_ERROR);
    				returnStatus.addDetail(CommerceLinkFactory.STATUS_CODES.RESPONSE_OBJECT, result.object);
				}
			}
		} catch(e) {
